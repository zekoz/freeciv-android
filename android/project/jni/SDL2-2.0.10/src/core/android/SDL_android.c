...
char **Android_JNI_ListAssets(const char *dir)
{
    char **asset_filenames;
    jsize i, size;
    JNIEnv *env = Android_JNI_GetEnv();
    jmethodID mid;
    jobject assetManager;
    ...
    * This method reads the files directly from the APKs using standard *nix calls
    */
    mid = (*env)->GetMethodID(env, (*env)->GetObjectClass(env, assetManager), "list",
        "(Ljava/lang/String;)[Ljava/lang/String;");

    string = (*env)->NewStringUTF(env, dir);

    assets = (*env)->CallObjectMethod(env, assetManager, mid, string);

    size = (*env)->GetArrayLength(env, (jarray)assets);
    asset_filenames = malloc((size + 1) * sizeof(char*));
    for (i = 0; i < size; i++) {
        jstring s = (*env)->GetObjectArrayElement(env, assets, i);
        const char *c = (*env)->GetStringUTFChars(env, s, NULL);
        asset_filenames[i] = strdup(c);
        (*env)->ReleaseStringUTFChars(env, s, c);
        (*env)->DeleteLocalRef(env, s);
    }
    asset_filenames[i] = NULL;
